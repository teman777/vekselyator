...
class Operations:
    def __init__(self, id: int = 0, userTo: List[int] = [], userFrom: int = 0, qty: float = 0, chatId: int = 0,
                 comment: str = '', type: int = 1):
        self.userTo = userTo
    ...
    def save(self):
        ...
        if not exists:
            lastid = db.insert('Operations',
                               {'ChatId': self.chatId, 'UserFrom': self.userFrom, 'UserTo': str(self.userTo),
                                'Qty': self.qty, 'Type': self.type, 'Comment': self.comment})
            self.id = lastid
        else:
            db.update('Operations', self.id,
                      {'UserTo': str(self.userTo), 'UserFrom': self.userFrom, 'Qty': self.qty, 'Type': self.type,
                       'Comment': self.comment, 'ChatId': self.chatId})
    ...
    def resolve(self):
        ...
        for i in self.userTo:
            if i != self.userFrom:
                operation = Operation(userTo=i, userFrom=self.userFrom, qty=calc_qty, chatId=self.chatId,
...
class Operation:
    ...
    def save(self):
        ...
        if not exists or self.id == 0:
            lastid = db.insert('Operation',
                               {'UFrom': self.userFrom, 'UTo': self.userTo, 'Qty': self.qty, 'Comment': self.comment,
                                'ChatID': self.chatId, 'Date': self.date})
            self.id = lastid
        elif exists:
            db.update('Operation', self.id,
                      {'UFrom': self.userFrom, 'UTo': self.userTo, 'Qty': self.qty, 'Comment': self.comment,
                       'ChatID': self.chatId})
    ...
    def load(self):
        ...
        for o in op:
            self.operations.append(
                Operation(id=int(o[0]), userFrom=int(o[1]), userTo=int(o[2]), qty=float(o[3]), comment=o[4],
                          chatId=int(self.id)))
...
def deleteAllOperation(chat_id: int):
    db.massDeleteOperations(chat_id)


def getSuperSaldo(chat_id: int) -> List[Operation]:
    res = checkAndRepairSum(db.getSuperSaldo(chat_id))
    operations = []
    while min(res, key=lambda a: a[1])[1] != 0:
        minPair = min(res, key=lambda a: a[1])
        maxPair = max(res, key=lambda a: a[1])
        qty = abs(min(minPair, maxPair, key=lambda a: abs(a[1]))[1])
        oper = Operation(userTo=minPair[0]
                         , userFrom=maxPair[0]
                         , qty=qty
                         , chatId=chat_id
                         , comment='Суперсальдо')
        ...
        if -qty == minPair[1]:
            minQty = 0
            maxQty = maxPair[1] - qty
        else:
            minQty = minPair[1] + qty
            maxQty = 0
        res.remove(minPair)
        res.remove(maxPair)
        res.append((minPair[0], minQty))
        res.append((maxPair[0], maxQty))
    return operations


def checkAndRepairSum(pairs: list[tuple]) -> list[tuple]:
    if len(pairs) == 0:
        return
    check = sum(float(p) for id, p in pairs)
    if check != 0:
        pairs[0][1] = pairs[0][1] - check
    return pairs
